let theWords = ['SUPERNOVA', 'UNIVERSE', 'PLANETS', 'MARS', 'STAR', 'MOON', 'EARTH', 'VENUS', 'SATURN', 'MERCURY']
var theme = sessionStorage.getItem("thememe");
if (theme == "animal") {
  theWords = ['CHIMERA', 'GRIFFIN', 'HARPY', 'WYVERN', 'CENTAUR', 'DRAGON', 'UNICORN', 'SATYR', 'SPHINX', 'PHOENIX'];
}
else if (theme == "movie") {
  theWords = ['HARRYPOTTER', 'NOTEBOOK', 'TITANIC', 'DRSTRANGE', 'AVENGERS', 'BATMAN', 'DEADPOOL', 'JOCKER', 'TWILIGHT', 'INCEPTION'];
}
else {
  theWords = ['SUPERNOVA', 'UNIVERSE', 'PLANETS', 'MARS', 'STAR', 'MOON', 'EARTH', 'VENUS', 'SATURN', 'MERCURY'];
}
  ...
  WordSearch.prototype.lookup = function (selected) {
    ...
    if (
      ...
      for (var i = 0; i < wordListItems.length; i++) {
        if (words[0] == wordListItems[i].innerHTML) {
          if (
